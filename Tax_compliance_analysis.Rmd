---
title: "Increasing Property Tax Payment Rates on Time Through Simplification of Communication with Citizens"
...
This file replicates the analysis of the following paper: "*Increasing Property Tax Payment Rates on Time Through Simplification of Communication with Citizens*" submitted to [Journal of Economic Behavior & Organization](https://www.sciencedirect.com/journal/journal-of-economic-behavior-and-organization). The file replicates the results shown in the main text as well as in the appendix.
...
# ggsave(plot_inter_gender, width = 20, height = 12, units = c("cm"), file ="plot_inter_gender.jpg")
```
...
plot_inter_age_cont <- model_HTE_age_inc %>% 
  ...
  labs(y = expression(atop("Predicted Treatment Effect (95% CI)", italic("Property Tax Payment (single payment or instalments)"), 
      )),  x = "Age") +
...
# Basic Model with robust SE (DV = Tax_paid_ontime_full)
model1_full <- lm_robust(Tax_paid_ontime_full ~ Treatment_full, 
                    data = data_subset_filtered, se_type = "HC3")

# Model with covariates and robust SE (DV = Tax_paid_ontime_full)
model2_full <- lm_robust(Tax_paid_ontime_full ~ Treatment_full + Tax_total + Tax_declaration_21 + Gender + Age + I(Age^2), data = data_subset_filtered, se_type = "HC3")
